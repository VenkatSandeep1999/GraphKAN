...
def get_train_test_data(conf, need_road_network_structure_matrix):
    ...
    train_gen_weight, test_gen_weight = data.split(
        conf.test_ratio,
        [xs, ys, arms, xp, xt, xe, vehicle_type, engine_config, gen_weight]
    )
    
    print('************* split  **********************')
    print(
        'arms:', arms.shape,
        'train_xs:', train_xs.shape,
        'test_xs:', test_xs.shape,
        'train_ys:', train_ys.shape,
        'test_ys:', test_ys.shape,
        'train and test _xp:', train_xp.shape, test_xp.shape,
        'train and test _xt:', train_xt.shape, test_xt.shape,
        'train and test _xe:', train_xe.shape, test_xe.shape
    )

    train_xs, train_ys, train_arms, train_xp, train_xt, train_xe, \
    train_vehicle_type, train_engine_config, train_gen_weight = get_multiple_equal_batch_size(
        [
            train_xs,
            train_ys,
            train_arms,
    ...
    test_vehicle_type, test_engine_config, test_gen_weight = get_multiple_equal_batch_size(
        [
            test_xs,
            test_ys,
            test_arms,
    ...
    if need_road_network_structure_matrix:
        ...
        return (
            data, arm_shape, train_xs, train_ys, None, train_xp, train_xt, train_xe,
            train_vehicle_type, train_engine_config, train_gen_weight,
...
if __name__ == '__main__':
    ...
    train_vehicle_type, train_engine_config, train_gen_weight, \
    test_xs, test_ys, test_arms, test_xp, test_xt, test_xe, \
    test_vehicle_type, test_engine_config, test_gen_weight = get_train_test_data(
        conf,
        need_road_network_structure_matrix=True
    )
